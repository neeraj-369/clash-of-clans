...
class archer:
    def spawn(self, values, symbol):
        if(symbol == 'j' or symbol == 'J'):
            if(values.archer_count < 6):
                values.archer_done[values.archer_count] = 1
                values.archer_alive[values.archer_count] = 1
                x = 2
                y = 80
                values.archers.append([x, y])
                values.frame[x][y] = values.archer_symbol
                values.archer[values.archer_count] = 1
                values.archer_count += 1
    ...
    def nearest(self, i, values):
        ...
        x = values.archers[i][0]
        y = values.archers[i][1]
        main_list = []
        min_dist = []
...
#################
    ...
    def color(self, x, y, values, i):
        if(values.archer_health[i] >= 50):
            values.frame[x][y] = f"{Fore.GREEN}{Style.BRIGHT}a{Style.RESET_ALL}"
        if(values.archer_health[i] >= 20 and values.archer_health[i] < 50):
            values.frame[x][y] = f"{Fore.YELLOW}{Style.BRIGHT}a{Style.RESET_ALL}"
        if(values.archer_health[i] < 20):
            values.frame[x][y] = f"{Fore.RED}{Style.BRIGHT}a{Style.RESET_ALL}"

    def automate(self, i, values, hut_1, hut_2, hut_3, hut_4, townhall, cannon, wizzard):
        coordinates = self.nearest(i, values)
        x = values.archers[i][0]
        y = values.archers[i][1]
        if(coordinates != -1 and math.sqrt((coordinates[0]-x) ** 2 + (coordinates[1]-y)**2) <= values.rangeofarcher):
            ii = coordinates[0]
            ji = coordinates[1]
            value = values.object_near[ii][ji]
            if(value[0:3] == 'hut'):
            ...
            if(value == 'townhall'):
                values.townhall_health = values.townhall_health-values.archer_hit
            ...
            if(coordinates != -1):
                if(coordinates[0] < x and coordinates[1] < y):
                    ...
                    if l in values.available:
                        values.frame[x][y] = " "
                        # values.frame[x-1][y-1] = values.archer_symbol
                        self.color(x-1, y-1, values, i)
                        values.archers[i][0] = x-1
                        values.archers[i][1] = y-1
                    else:
                        if (values.object_near[x-1][y-1] == 'wall'):

                            if (values.object_near[x-1][y-1] == 'wall'):
                                values.frame[x][y] = " "
                                # values.frame[x][y-1] = values.archer_symbol
                                self.color(x-1, y-1, values, i)
                                values.archers[i][0] = x-1
                                values.archers[i][1] = y-1
                                ...
                                values.frame[x][y] = " "
                                # values.frame[x-1][y] = values.archer_symbol
                                self.color(x-1, y, values, i)
                                values.archers[i][0] = x-1
                                values.archers[i][1] = y
