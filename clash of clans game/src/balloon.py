...
class balloon:
    def spawn(self, values, symbol):
        if(symbol == 't' or symbol == 'T'):
            if(values.balloon_count < values.numberofballoons):
                values.balloon_done[values.balloon_count] = 1
                values.balloon_alive[values.balloon_count] = 1
                x = 2
                y = 85
                values.balloons.append([x, y])
                values.frame[x][y] = values.balloon_symbol
                values.balloon[values.balloon_count] = 1
                values.balloon_count += 1
    ...
    def nearest(self, i, values):
        ...
        x = values.balloons[i][0]
        y = values.balloons[i][1]
        main_list = []
        min_dist = []
...
#################
    ...
    def color(self, x, y, values, i):
        if(values.balloon_health[i] >= 50):
            values.frame[x][y] = f"{Fore.GREEN}{Style.BRIGHT}b{Style.RESET_ALL}"
        if(values.balloon_health[i] >= 20 and values.balloon_health[i] < 50):
            values.frame[x][y] = f"{Fore.YELLOW}{Style.BRIGHT}b{Style.RESET_ALL}"
        if(values.balloon_health[i] < 20):
            values.frame[x][y] = f"{Fore.RED}{Style.BRIGHT}b{Style.RESET_ALL}"
    ...
    def automate(self, i, values, hut_1, hut_2, hut_3, hut_4, townhall, cannon, wizzard):
        ...
        if(coordinates != -1):
            if(coordinates[0] < x and coordinates[1] < y):
                values.frame[x][y] = values.balloon_prev[i]
                if(x-1 == coordinates[0] and y-1 == coordinates[1]):
                    self.color(x, y-1, values, i)
                    values.balloons[i][0] = x
                    values.balloons[i][1] = y-1
                else:
                    x = x-1
                    y = y-1
                values.balloon_prev[i] = values.frame[x][y]
                if(values.balloon_prev[i] == values.balloon_symbol):
                    values.balloon_prev[i] = ' '
                    if(values.object_near[x][y] == 'wall'):
                        values.balloon_prev[i] = values.wall_symbol
                values.frame[x][y] = values.balloon_symbol
                self.color(x, y, values, i)
                values.balloons[i][0] = x
                values.balloons[i][1] = y
                x = xx
                y = yy
            ...
            if(coordinates[0] == x):
                if(coordinates[1] < y):
                    if(x == coordinates[0] and y - 1 == coordinates[1]):
                        ...
                        if(value == 'townhall'):
                            values.townhall_health = values.townhall_health-values.balloon_hit
                        ...
                        values.frame[x][y] = values.balloon_prev[i]
                        values.balloon_prev[i] = values.frame[x][y-1]
                        if(values.balloon_prev[i] == values.balloon_symbol):
                            values.balloon_prev[i] = ' '
                            if(values.object_near[x][y-1] == 'wall'):
                                values.balloon_prev[i] = values.wall_symbol
                        values.frame[x][y-1] = values.balloon_symbol
                        self.color(x, y-1, values, i)
                        values.balloons[i][0] = x
                        values.balloons[i][1] = y-1
                ...
                if(coordinates[0] < x):
                    if(x-1 == coordinates[0] and y == coordinates[1]):
                        ...
                        values.frame[x][y] = values.balloon_prev[i]
                        values.balloon_prev[i] = values.frame[x-1][y]
                        if(values.balloon_prev[i] == values.balloon_symbol):
                            values.balloon_prev[i] = ' '
                            if(values.object_near[x-1][y] == 'wall'):
                                values.balloon_prev[i] = values.wall_symbol
                        values.frame[x-1][y] = values.balloon_symbol
                        self.color(x-1, y, values, i)
                        values.balloons[i][0] = x-1
                        values.balloons[i][1] = y
